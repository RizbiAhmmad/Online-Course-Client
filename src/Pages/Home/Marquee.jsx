...
function Marquee({
  ...
  return <div {...props} ref={containerRef} className={cn("group flex overflow-hidden p-2 [--gap:2rem] gap-(--gap)" + (vertical ? " flex-col" : " flex-row"), className)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div ref={contentRef} className={cn("flex shrink-0 justify-around gap-(--gap)" + (vertical ? " flex-col" : " flex-row"))}>
        {Array(repeat).fill(0).map((_, i) => <div key={i} ref={i === 0 ? singleContentBlockRef : null} className="flex gap-8">
              {children}
            </div>)}
      </div>
    </div>;
